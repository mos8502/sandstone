...
class TaskListsFragment @Inject constructor(viewModelFactory: ViewModelProvider.Factory) :
    Fragment() {
    private val viewModel by viewModels<TaskListsViewModel> { viewModelFactory }
    private val adapter = TaskListsAdapter()
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewModel.taskLists.observe(viewLifecycleOwner, Observer { lce ->
            if (lce is Lce.Content<List<TaskList>>) {
                adapter.submitList(lce.value)
            }
        })
    }
}